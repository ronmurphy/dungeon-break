...
2-24-26:
BIG CHANGE: Camp / cellular automata, always level 1, wanderers can be adjusted in code as needed, more hills, less holes, setting upop for the base plot to the entire game, the campfire with Joe and Old Pell.  day and night in now, along with wanderers spawning from shadows and at night, the campfire and the azure flame literally stop them form coming in and hurting you.  night spawned wanderers, about half vanish when daylight comes. fog disabled permently in camp, could not get it to work right.  it's a small map anyway, 70x70 something units.  when the torch gets low it turns off, there is a torch toggle in inventory, need to refill it. wanderers dont jump, try to avoid holes. the Duck is back, lucky duck, duck treasure, new items and old items redone.  rebiuilt the bonfire code to the trio >joe, mira, old pell< each have an offering.  also, Elite wanderers, +50% bigger and +50% stats boost, 1 out of 10 is an elite. Ranger's mask and cultists's mask act as head armor with effects.

2-25-26:

a bad code pull or github, idk, messed things up, doing emergency management and recovery.  time indicator and time on all three maps, ca, bsp and battle island. torch button actually does on / on now with a small night vision for the player when torch = off. fixed bsp-dungeon 'room' no move issues. 