...
import { initMinigames, startLockpickGame, startPotionGame } from './minigames.js';
...
let isDebugWandererActive = false; // Track if wanderer debug is on
...
function init3D() {
    ...
    initMinigames({
        loadTexture,
        loadFXImage, // Use local function
        enterRoom,
        closeCombat,
        takeDamage,
        gameOver,
...
window.showWandererDebug = function(show) {
    ...
    wanderers.forEach((w, idx) => {
        ...
        // 1. Combat Trigger Radius (Red) - 1.2
        const combatGeo = new THREE.RingGeometry(1.15, 1.2, 32);
        const combatMat = new THREE.MeshBasicMaterial({ color: 0xff0000, side: THREE.DoubleSide, depthTest: false });
        const combatRing = new THREE.Mesh(combatGeo, combatMat);
        combatRing.rotation.x = -Math.PI / 2;
        // Position set in update loop
        scene.add(combatRing);
        w.debugCombatRing = combatRing;
        debugHelpers.push(combatRing);

        // 2. Vision Range (Yellow) - 4.0
        const visionGeo = new THREE.RingGeometry(3.95, 4.0, 32);
        const visionMat = new THREE.MeshBasicMaterial({ color: 0xffff00, side: THREE.DoubleSide, depthTest: false });
        const visionRing = new THREE.Mesh(visionGeo, visionMat);
        visionRing.rotation.x = -Math.PI / 2;
        // Position set in update loop
        scene.add(visionRing);
        w.debugVisionRing = visionRing;
        debugHelpers.push(visionRing);

        // 3. Facing Direction (Cyan Arrow)
        const forward = new THREE.Vector3();
        w.mesh.getWorldDirection(forward);
        forward.y = 0; forward.normalize();

        const arrow = new THREE.ArrowHelper(forward, w.mesh.position, 2.0, 0x00ffff);
        scene.add(arrow);
        w.debugArrow = arrow;
...
function updateWandererDebug() {
    ...
    wanderers.forEach((w, idx) => {
        ...
        if (w.debugCombatRing) {
            ...
            w.debugCombatRing.position.y += 0.1;
        }
        if (w.debugVisionRing) {
            w.debugVisionRing.position.copy(w.mesh.position);
            w.debugVisionRing.position.y += 0.1;
        }
        ...
        if (w.debugArrow) {
            w.debugArrow.position.copy(w.mesh.position);
            w.debugArrow.setDirection(forward);
        }
    });
}
