...
class Node_ {
    ...
    split(iterationCount: number): void {
        ...
        if (iterationCount > 0) {
            if (
                this.left.chunk.w > this.left.chunk.minWidth &&
            ...
            if (
                this.right.chunk.w > this.right.chunk.minWidth &&
    ...
    generateRooms(): void {
        if (!this.isSplit) {
            let x: number = Math.floor(
                this.chunk.x + this.random(this.chunk.w * 0.2, this.chunk.w * 0.25)
            );
            let y: number = Math.floor(
                this.chunk.y + this.random(this.chunk.h * 0.2, this.chunk.h * 0.25)
            );

            let w: number = Math.floor(
                this.random(this.chunk.w * 0.6, this.chunk.w * 0.7)
            );
            let h: number = Math.floor(
                this.random(this.chunk.h * 0.6, this.chunk.h * 0.7)
            );
...
class Visualization {
        ...
        this.steps = [
            ...
            () => {
                if (this.mobCounter < this.mobs.length) {
                    ...
                    this.ctx.fillText(mob.char, mob.x * this.scaleX, mob.y * this.scaleY);
                    this.mobCounter++;
                } else {
                    this.stepCounter++;
                }
    ...
    getMobs(): { x: number; y: number; char: string }[] {
        ...
        for (let i: number = 0; i < this.mobCount; i++) {
            if (floors.length > 0) {
                let floor: { x: number; y: number } = floors.splice(
                    Math.floor(Math.random() * floors.length),
                                                                    1
                )[0];
                mobs.push({
                    x: floor.x,
...
document.querySelector(".screen") || new HTMLCanvasElement();
...
const statusInfo: HTMLElement =
document.querySelector(".status") || new HTMLElement();
...
// ---------------- Dom
const generateBtn: HTMLButtonElement =
document.querySelector(".button") || new HTMLButtonElement();
...
document.querySelector<HTMLInputElement>(".size") || new HTMLInputElement();
sizeInput.addEventListener("input", function () {
    inputs.COL = Number(this.value);
    inputs.ROW = Number(this.value);

    const v_input: HTMLParagraphElement =
    document.querySelector(`.${this.getAttribute("data-v")}`) ||
    new HTMLParagraphElement();
    v_input.innerHTML = this.value;
});

const otherRanges: HTMLInputElement[] =
[...document.querySelectorAll<HTMLInputElement>(".range_input")] || [];
